...
org_loc_object = Oracle(logger_object)
queryString = extraction_query_generator(table_name)
data = org_loc_object.fetchAll(queryString)
columns = org_loc_object.fetchHeader(queryString)
...
# Getting the org_loc to save the extracted data file in local machine
...
org_loc_snowflake.executequery(stage_upload_query)
...
stage_table_creation_query = f"""
                    CREATE TABLE IF NOT EXISTS {table_name} (
                      ...
                      ORG_ID INT,
                      ORG_DESC VARCHAR(40),
                      ORG_IS_ACTIVE_FLAG BOOLEAN
                    )"""
org_loc_snowflake.executequery(stage_table_creation_query)
...
org_loc_snowflake.executequery(stage_table_truncate_query)
...
org_loc_snowflake.executequery(stage_table_load_query)
...
org_loc_object.disconnect()
org_loc_snowflake.disconnect()